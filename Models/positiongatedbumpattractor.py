...
plt.figure()
plt.plot(cuediffs, perf)
plt.xlabel('#L - #R')
plt.ylabel('Went Left')
...
#priordata = np.load('bumpFFto800.npy')
#alldata = np.concatenate((priordata, alldata), axis=2)
...
plt.savefig('MovedActivationNonNormBumpAttractor-faster.pdf', transparent = True)
...
plt.savefig('MovedActivationBumpAttractor-faster.pdf', transparent = True)

#np.save('bumpmodelfull-feedforwardinside-faster.npy', alldata)
