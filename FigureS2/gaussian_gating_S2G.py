...
#Gaussian function
def gauss(t, mus, sig = 25, a = 60):
    return a*np.exp((-(t-mus)**2)/(2*sig**2))


#Gaussian position gate
def P(t):
    mus = np.arange(-10, 330, 20)
    pos = gauss(t, mus)
...
plt.figure()
plt.axvspan(35, 50, color = 'cyan', alpha = .2)
plt.axvspan(70, 85, color = 'cyan', alpha = .4)
plt.plot(times[start:stop], Pvals[:, 5][start:stop], color = 'cyan', label = 'i-1')
plt.plot(times[start:stop], Pvals[:, 4][start:stop], linestyle = 'dashdot', color = 'cyan', linewidth = 3, label = 'i')
plt.plot(times[start:stop], Pvals[:, 6][start:stop], linestyle = '--', color = 'cyan', linewidth = 2.5, label = 'i+1')
plt.legend()
