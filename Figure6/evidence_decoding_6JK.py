...
for region, matfiles in zip(regions, filelist):
    ...
    decodingleftr2 = np.zeros((len(matfiles), 66))
    decodingleftr = np.zeros((len(matfiles), 66))
    decodingleftcorr = np.zeros((len(matfiles), 66))
    bimodalleft = np.zeros((len(matfiles), 66))
    bimodallefttrue = np.zeros((len(matfiles), 66))
    decodingleftcorrS = np.zeros((shufflereps*len(matfiles), 66))
    decodingleftr2S = np.zeros((shufflereps*len(matfiles), 66))
    decodingleftrS = np.zeros((shufflereps*len(matfiles), 66))
    
    decodingrightr2 = np.zeros((len(matfiles), 66))
    decodingrightr = np.zeros((len(matfiles), 66))
    decodingrightcorr = np.zeros((len(matfiles), 66))
    bimodalright = np.zeros((len(matfiles), 66))
    bimodalrighttrue = np.zeros((len(matfiles), 66))
    decodingrightcorrS = np.zeros((shufflereps*len(matfiles), 66))
    decodingrightr2S = np.zeros((shufflereps*len(matfiles), 66))
    decodingrightrS = np.zeros((shufflereps*len(matfiles), 66))
        ...
        if len(correcttrials)>10:
            ...
            for p in range(6, n_pos):
                ...
                if len(validns)>0:
                    ...
                    #impute missing values before making transformations
                    imputer = KNNImputer()
